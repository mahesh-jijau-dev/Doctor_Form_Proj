                ...
                <div class="public-meta-row">
                    <div class="public-field-group public-field-half">
                        ...
                        <input id="_name" name="_name" type="text" value="{{ old('_name') }}"
                            class="public-input" placeholder="Enter your full name" />
                    </div>
                    <div class="public-field-group public-field-half">
                        <label for="_email" class="public-label">Email Address</label>
                        <input id="_email" name="_email" type="email" value="{{ old('_email') }}"
                            class="public-input" placeholder="Enter your email" />
                    </div>
                </div>
                            ...
                            <div class="public-question">
                                ...
                                @if ($field->type === 'short_text')
                                    <input id="{{ $fieldName }}" name="{{ $fieldName }}" type="text"
                                        ...
                                        class="public-input {{ $hasError ? 'is-invalid' : '' }}" />
                                @elseif($field->type === 'long_text')
                                    <textarea id="{{ $fieldName }}" name="{{ $fieldName }}" rows="4"
                                        placeholder="{{ $field->placeholder ?? 'Your answer' }}"
                                        class="public-input public-textarea {{ $hasError ? 'is-invalid' : '' }}">{{ old($fieldName) }}</textarea>
                                ...
                                @elseif($field->type === 'phone')
                                    <input id="{{ $fieldName }}" name="{{ $fieldName }}" type="tel"
                                        value="{{ old($fieldName) }}"
                                        placeholder="{{ $field->placeholder ?? '+91 98765 43210' }}"
                                        class="public-input {{ $hasError ? 'is-invalid' : '' }}" />
                                @elseif($field->type === 'number')
                                    <input id="{{ $fieldName }}" name="{{ $fieldName }}" type="number"
                                        value="{{ old($fieldName) }}" placeholder="{{ $field->placeholder ?? '123' }}"
                                        class="public-input {{ $hasError ? 'is-invalid' : '' }}" />
                                ...
                                @elseif($field->type === 'time')
                                    <input id="{{ $fieldName }}" name="{{ $fieldName }}" type="time"
                                        value="{{ old($fieldName) }}"
                                        class="public-input {{ $hasError ? 'is-invalid' : '' }}" />
                                    ...
                                    <select id="{{ $fieldName }}" name="{{ $fieldName }}"
                                        ...
                                        @foreach ($field->options as $option)
                                            <option value="{{ $option->value ?: $option->label }}"
                                                {{ old($fieldName) == ($option->value ?: $option->label) ? 'selected' : '' }}>
                                                {{ $option->label }}
                                            </option>
                                        @endforeach
                                    </select>
                                        ...
                                        @foreach ($field->options as $option)
                                            <label class="public-option-item">
                                                <input type="radio" name="{{ $fieldName }}"
                                                    ...
                                                    {{ old($fieldName) == ($option->value ?: $option->label) ? 'checked' : '' }}>
                                        ...
                                        @foreach ($field->options as $option)
                                            <label class="public-option-item">
                                                <input type="checkbox" name="{{ $fieldName }}[]"
                                                    value="{{ $option->value ?: $option->label }}"
                                                    {{ is_array(old($fieldName)) && in_array($option->value ?: $option->label, old($fieldName)) ? 'checked' : '' }}>
                                                <span>{{ $option->label }}</span>
                                            </label>
                                        @endforeach
                                    </div>
            ...
            return {
                ...
                evaluate(fieldId) {
                    ...
                    for (const c of conds) {
                        ...
                        switch (c.operator) {
                            case 'equals':
                                match = vStr === String(c.condition_value);
                                break;
                            case 'not_equals':
                                match = vStr !== String(c.condition_value);
                                break;
                            case 'contains':
                                match = vStr.includes(c.condition_value);
                                break;
                            case 'is_empty':
                                match = !vStr || vStr === '';
                                break;
